...
export const Question1 = (props) => {
  ...
  const handleShow = () => setShow(true);
    ...
    const handleSubmit = e => {
        ...
        const dob = moment(resstate.dob, "YYYY-MM-DD");
        const now = moment()
        let year = now.diff(dob, 'years'); // returns 1
        console.log(year)

        if (year == 2) {
            props.history.push('/gendev2/');
        }
        else if (year == 3) {
            props.history.push('/gendev3/')
        }
        else if (year == 4) {
            props.history.push('/gendev4/')
        }
        else if (year == 5) {
            props.history.push('/gendev5/')
        }

        else if (year > 5) {
            props.history.push('/above5/')
        } else if (year < 2) {
            handleShow()
        }
